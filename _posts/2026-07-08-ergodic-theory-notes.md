...
They are largely structured around two textbooks: *Operator-Theoretic Aspects of Ergodic Theory* [1] and *An Introduction to Ergodic Theory* [2], as well as lectures given by Dimitris Giannakis at Dartmouth College in Spring 2025. They were written for the Mathematics Directed Reading Program (DRP) at Dartmouth in the Winter of 2026.
...
<figure>
  <img src="/images/fig_circle_rotation.png" alt="Orbits of a circle rotation, drawn for a rational and an irrational rotation angle.">
  <figcaption>Orbits of the circle rotation $z \mapsto \alpha z$ on $S^1$. When $\alpha$ is rational every orbit closes up; when $\alpha$ is irrational, orbits are dense.</figcaption>
</figure>

<figure>
  <img src="/images/fig_doubling_map.png" alt="The doubling map on the unit interval: its graph with a trajectory, and its action on binary expansions.">
  <figcaption>The doubling map on $[0,1]$. Left: the graph of $T(x) = 2x \bmod 1$ with a trajectory. Right: on binary expansions, $T$ acts as the left shift.</figcaption>
</figure>

<figure>
  <img src="/images/fig_gauss_map.png" alt="The Gauss map with its invariant density overlaid.">
  <figcaption>The Gauss map $T(x) = 1/x - \lfloor 1/x \rfloor$ with its invariant density $\frac{1}{(1+x)\log 2}$ overlaid. The map's branches on $(1/(n+1), 1/n]$ record the terms of the continued fraction expansion of $x$.</figcaption>
</figure>
...
**Theorem 1.4 (Poincaré Recurrence).** Given a measure-preserving dynamical system $(X, \mathcal{X}, \mu, T)$ with $\mu$ finite, for any set $E \in \mathcal{X}$, the set of points
...
**(2) $\Rightarrow$ (3):** Consider the set $A = \bigcup_{k>0} T^{-k}(S)$. Then $T^{-1}(A) \subseteq A$, and $\mu(T^{-1}(A)) = \mu(A)$ since $T$ is measure-preserving, so $\mu(A \setminus T^{-1}(A)) = 0$, giving $\mu(A\, \Delta\, T^{-1}(A)) = 0$. By (2), $\mu(A) = 0$ or $1$. But if $\mu(S) > 0$, we have $S \subseteq A$, and so $\mu(A) = 1$.
...
<figure>
  <img src="/images/fig_ergodic_decomposition.png" alt="Left: a non-ergodic system splitting into two invariant pieces. Right: an ergodic system.">
  <figcaption>Ergodicity as irreducibility. Left: a non-ergodic system whose state space splits into two invariant pieces. Right: an ergodic system, where almost every orbit visits every part of the space.</figcaption>
</figure>
...
If $A \in \mathrm{GL}_n(\mathbb{Z})$, then it induces a map $\mathscr{T}_A : \mathbb{T}^n \to \mathbb{T}^n$ preserving the Lebesgue measure induced on $\mathbb{T}^n = \mathbb{R}^n / \mathbb{Z}^n$ by noting that the action of $A: \mathbb{R}^n \to \mathbb{R}^n$ preserves $\mathbb{Z}^n$. These are the **toral automorphisms**.

**Theorem 2.6.** $\mathscr{T}_A$ is ergodic if and only if no eigenvalue of $A$ is a root of unity.

If a toral automorphism has no eigenvalues of modulus $1$, we say it is a **hyperbolic automorphism**. People like studying these.

<figure>
  <img src="/images/fig_cat_map.png" alt="Arnold's cat map applied repeatedly to an image on the torus, smearing it across the whole space.">
  <figcaption>Arnold's cat map, a hyperbolic toral automorphism given by $A = \begin{pmatrix}2 & 1 \\ 1 & 1\end{pmatrix}$ acting on $\mathbb{T}^2$. Even a few iterates smear the original image across the whole torus.</figcaption>
</figure>
...
<figure>
  <img src="/images/fig_borel_normal.png" alt="For a typical point of the unit interval, the running proportion of ones in its binary expansion tends to one half.">
  <figcaption>Borel's normal-number theorem in action: for a typical $x \in [0,1)$, the running proportion of $1$s in the binary expansion tends to $1/2$.</figcaption>
</figure>
...
Let $A_N f = \frac{1}{N}\sum_{n=1}^N f \circ T^n$. Let $f_+(x) = \limsup_N A_N f(x)$ and $f_-(x) = \liminf_N A_N f(x)$. It suffices to show that for any $\varepsilon > 0$, we have that $f_+(x) < \int f\, d\mu + \varepsilon$ for almost all $x$, since we can then apply the same argument to $g = 1 - f$ to get an estimate for $f_-$.
...
Now, let $\alpha = \int f\, d\mu + \varepsilon$ and let $f_n(x) = f(x) + f(T(x)) + \cdots + f(T^{n-1}(x)) - n\alpha$, and let $F_N = \max\\{0, f_1, f_2, \ldots, f_N\\}$. Let $B_N$ denote the set $\\{x \in X : F_N(x) > 0\\}$. Then, clearly, for $x \in B_{N+1}$, for $n \leq N$, we have that $F_N(x) \geq f_n(x)$, and so $F_N(Tx) \geq f_n(Tx)$. This further implies that $F_N(Tx) + f(x) \geq f_n(Tx) + f(x) = f_{n+1}(x) + \alpha$. Taking a maximum over all $n$, we see that
...
It is fairly easy to check (and we will do so shortly) that the rotation map $z \mapsto \alpha z$ is ergodic but not weak-mixing (when $\alpha$ is not a root of unity). An example of a map that is weak-mixing but not strong-mixing is somewhat more difficult to give explicitly, but an example of this was given by Chacon (1969) using a cutting-and-stacking argument [3].
...
&= \lim_{N \to \infty} \frac{1}{N} \sum_{n=0}^{N-1} \int_{S^1} \lambda^n\, d\mu_f(\lambda) \int_{S^1} \overline{\tau}^n\, d\mu_f(\tau) \\
...
<figure>
  ...
  <figcaption>The Benford distribution over leading digits: $P(d) = \log_{10}(1 + 1/d)$. The digit $1$ appears roughly $30\%$ of the time, while $9$ appears less than $5\%$.</figcaption>
</figure>
